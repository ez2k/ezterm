...
![ezterm — 왼쪽 워크스페이스 사이드바, 가운데 쉘, 오른쪽 파일 매니저](docs/screenshots/ezterm.webp)
...
- `d` 다운로드 · `u` 업로드: **OS 기본 파일 대화상자**로 저장 위치/업로드할 파일을 고릅니다. macOS에서는 앱 내부에서 실행되는 진짜 `NSOpenPanel`/`NSSavePanel`이라 터미널 창에 붙어서 뜹니다 (Windows는 내장 대화상자, Linux는 `zenity`/`kdialog`; 없으면 경로 입력·다운로드 폴더로 대체). 전송 진행률 표시.
- 로컬 페인에서는 메뉴의 "Go to folder..."로 폴더 선택 대화상자를 열어 바로 이동할 수 있습니다.
...
- `m` 또는 **Shift/Ctrl+우클릭**: 열기/보기, 다운로드, 업로드, 이름 변경, 삭제, 상위, 뒤로/앞으로, 새로고침, 닫기 액션 메뉴.
- `F2` 이름 변경, `Delete` 삭제(확인 후; 디렉터리는 비어 있을 때만). 로컬·원격(SFTP) 모두 지원.
...
- **Shift를 누른 채 다른 탭 위에 드롭**하면 그 탭에 **합치기**(페인들이 분할로 들어감). 탭 메뉴의 "왼쪽/오른쪽 탭에 합치기"도 동일.
...
완료: 우클릭 컨텍스트 메뉴(터미널·탭·사이드바·파일 매니저), 워크스페이스 닫기 확인창, 탭 드래그(순서 변경·새 창 분리·워크스페이스 이동). 페인 드래그(분할·교체·탭/워크스페이스/새 창 이동), 탭 합치기, 파일 매니저 삭제/이름 변경. 이후 후보: 세션 복원, 드롭다운(Quake) 모드, 명령 완료 알림, 키 바인딩 치트시트.
