...
완료: 우클릭 컨텍스트 메뉴(터미널·탭·사이드바·파일 매니저), 워크스페이스 닫기 확인창, 탭 드래그(순서 변경·새 창 분리·워크스페이스 이동). 진행 중: 페인 드래그, 탭↔페인 합치기, 파일 매니저 삭제/이름 변경. 이후 후보: 세션 복원, 드롭다운(Quake) 모드, 명령 완료 알림, 키 바인딩 치트시트.
